...
class NewsLogic extends BaseLogic
{
    ...
    public function check($param)
    {
        ...
        if (!$param['news_id']) {
            ...
            $count = ContentNews::where(function ($query) use ($whereOr, $whereBase, $whereFiled) {
                $query->whereOr($whereOr);
            })->where($whereBase)->where($whereFiled)->count();
        }
        if ($count > 0) {
            return $this->resultFailed(ReturnCode::DATA_REPEAT, '标题或简介重复');
        }
